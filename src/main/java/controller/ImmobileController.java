...
public class ImmobileController {
    ...
    private static final Logger logger = Logger.getLogger(ImmobileController.class.getName());
        ...
        try {
            ...
            if (logger.isLoggable(Level.SEVERE))
                logger.log(Level.SEVERE,
                    String.format("Errore durante il caricamento dell'immobile: %s", imm),
                    e
                );
            return false;
        }
    ...
    public Immobile recuperaDettagli(long idImmobile) {
        try {
            ...
            if (logger.isLoggable(Level.SEVERE))
                logger.log(Level.SEVERE,
                    String.format("Errore nel recupero dei dettagli dell'immobile con ID: %s", idImmobile),
                    e
                );
            return null;
        }
    ...
    public int riempiTableRisultati(JTable tableRisultati, RicercaDTO ricercaDTO) {
        try {
            ...
            logger.log(Level.SEVERE, "Errore nel popolamento della tabella risultati immobili", e);
        }
        return 0;
    }
