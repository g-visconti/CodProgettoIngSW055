...
public class OAuthController {
    ...
    private static final String TITOLI_ERRORE = "Errore";
    ...
    public String handleOAuthLogin(String token, String provider) {
        if (token == null || token.isEmpty()) {
            showMessage("Token non valido", TITOLI_ERRORE, JOptionPane.ERROR_MESSAGE);
            return null;
        }
        ...
        try {
            ...
            if (account == null) {
                showMessage("Autenticazione con " + provider + " fallita", TITOLI_ERRORE, JOptionPane.ERROR_MESSAGE);
                return null;
            }

            final String email = account.getEmail();
            if (email == null || email.isEmpty()) {
                showMessage("Impossibile ottenere l'email da " + provider, TITOLI_ERRORE, JOptionPane.ERROR_MESSAGE);
                return null;
            }
            ...
            try {
                ...
                if (logger.isLoggable(Level.WARNING)) {
                    logger.log(Level.WARNING,
                        String.format("Utente già registrato o errore di registrazione: %s", e.getMessage()),
                        e
                    );
                }
            ...
            return email;
                ...
                if (logger.isLoggable(Level.SEVERE)) {
                    logger.log(Level.SEVERE,
                        String.format("Errore durante l'autenticazione OAuth con provider: %s", provider),
                        e
                    );
                }
    ...
    public boolean validateToken(String token, String provider) {
        try {
            ...
            if (logger.isLoggable(Level.WARNING)) {
                logger.log(Level.WARNING,
                    String.format("Token OAuth non valido o errore durante la validazione: %s", provider),
                    e
                );
            }
            return false;
    ...
    private void logAccesso(String email, String provider) {
        logger.info(String.format("[LOG] Accesso OAuth: %s via %s", email, provider));
    }
